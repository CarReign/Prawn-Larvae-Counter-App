    ...
    return (
        <View className="bg-[#ECF4FB] h-full flex flex-col justify-between">
            <View className="p-4 ">
                <View className="mb-2">
                    <Pressable
                        className="flex-row justify-center items-center h-10 rounded-md border-[#1F375D] border-[.3px] px-4"
            ...
            >
                    ...
                    <View className="flex flex-col justify-center relative p-4 w-full  content-center">
                        ...
                        <View className="bg-[#F9FCFF] flex flex-col justify-between p-4 mt-6 h-[640px] shadow-md rounded-md opacity-100">
                            ...
                            <View className="flex flex-row justify-center items-end mb-4 mt-4">
                                ...
                                <Text className="text-center text-[24px] pr-2 text-[#1F375D]">{resultResponse.count}</Text>
                            ...
                            >
                                ...
                                { !!addToSamePondLoading && <><ActivityIndicator color={"#1F375D"}/><Text className="text-[#1F375D] text-[16px]">  Adding...</Text></>}
                                <Text>  </Text>
                                { !addToSamePondLoading && <Text className="text-[#1F375D] text-[16px]">Add to previous pond</Text>}
                            </Pressable>
                        </View>
                    </View>
                </Modal>
                ...
                <ScrollView className="max-h-96">
                    {ponds ? ponds.map((pond, index) => (
                        <Pressable
                            key={index}
                            className={`flex-row justify-between px-6 py-4 bg-${selectedPond?.pond_id === pond.pond_id ? '[#C8E2F9]' : '[#D5E0F1]'} mb-2 rounded-lg items-center`}
                            onPress={() => {
                                setSelectedPond(pond);
                            }}
                        >
                            <Text className="font-semibold text-lg text-[#1F375D] border-[.3px] rounded-md border-[#1F375D] px-3 py-1">{pond.pondNumber || index + 1}</Text>
                            <Image
                                source={require("../../../assets/line.png")}
                                style={{ width: 0.5, height: 15, tintColor: "#9fb7cc", marginTop: 3 }}
                            />
                            <Text className="font-semibold text-lg text-[#1F375D]">{pond.total_count || "0"}</Text>
                            <Image
                                source={require("../../../assets/line.png")}
                                style={{ width: 0.5, height: 15, tintColor: "#9fb7cc", marginTop: 3 }}
                            />
                            <Text className="font-semibold text-lg text-[#1F375D]">{getFeedNeeded(pond.total_count || 0)} kg</Text>
                        </Pressable>
                    )) :
                ...
                <View className="flex items-center justify-center h-full">
                    <View className="bg-white w-3/4 p-4 rounded-lg flex">
                        <View className="flex mb-4 w-full justify-items-end">
                            <Text className="text-[16px] text-[#1F375D] flex">Count again for the same pond?</Text>
                        ...
                        <View className="flex flex-row justify-end mt-2">
                            <View  className="flex flex-row">
                                ...
                                <Pressable
                                    disabled={loading}
                                    className="mr-2 rounded h-[36px] px-5 text-center bg-[#1F375D] flex flex-row justify-center items-center"
        ...
        >
            ...
            <View className="flex-row justify-between bg-[#ECF4FB]] border-t-[.3px] border-[#1F375D] h-16 p-4 items-center">
                <View>
                    <Text className="text-lg font-bold text-[#1F375D]">Selected pond:</Text>
                    <Text className="italic text-lg text-[#1F375D]">{selectedPond ? `Pond ${selectedPond.pondNumber}` : 'No selected pond yet'}</Text>
                </View>
                <Pressable
