                        ...
                        counts ?
                            counts.map((count, index) => {
                                return <View key={index} className="flex flex-row justify-between px-3 pt-3 border-b-[.3px] border-[#9fbcd4]  ml-5 mr-5 pr-6">
                                    <Text className="text-[#1F375D]">{moment.tz(count.created_at, moment.tz.guess()).format("MM/D/YYYY")}</Text>
                                    <Image source={require('../../../../../assets/line.png')} style={{ width: .5, height: 15, tintColor: "#9fb7cc", marginTop: 3 }} />
                                    <Text className="text-[#1F375D]">{moment.tz(count.created_at, moment.tz.guess()).format("hh:mm a")}</Text>
                                    <Image source={require('../../../../../assets/line.png')} style={{ width: .5, height: 15, tintColor: "#9fb7cc", marginTop: 3 }} />
                                    <Text className="text-[#1F375D]">{ponds?.find((pond) => pond.pond_id === count.pond_id)?.pondNumber || "Not Known"}</Text>
                                    <Image source={require('../../../../../assets/line.png')} style={{ width: .5, height: 15, tintColor: "#9fb7cc", marginTop: 3 }} />
                                    <Pressable className="bg-[#1F375D] rounded-md py-1 px-3 mb-3 justify-center flex flex-row items-center" onPress={() => toggleResulPicture(count)}>
        ...
        <View className="flex flex-col justify-center relative p-4 w-full  content-center">
            <View className="bg-[#F9FCFF] flex flex-col justify-between p-4 mt-12 h-[530px] shadow-md rounded-md opacity-100">
                ...
                <View className="flex flex-row justify-center items-end mb-4 mt-4">
                    ...
                    <Text className="text-center text-[24px] pr-2 text-[#1F375D]">{ count.count}</Text>
